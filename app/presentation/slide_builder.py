...
def _to_rgb(value) -> RGBColor:
    if isinstance(value, RGBColor):
        return value
    return RGBColor.from_string(str(value))
...
def add_title(slide, title: str, section: str = "", theme: dict | None = None) -> None:
    ...
    paragraph.font.color.rgb = _to_rgb(theme["brand"])
    if section:
        ...
        tag.text_frame.paragraphs[0].font.color.rgb = _to_rgb(THEME["muted"])
...
def add_footer(slide, number: int, theme: dict | None = None) -> None:
    ...
    line.fill.solid()
    line.fill.fore_color.rgb = _to_rgb(theme["accent"])
    line.line.fill.background()
    footer = slide.shapes.add_textbox(MARGIN_X, Inches(7.18), Inches(12), Inches(0.18))
    brand = theme.get("brand_name", "UPS Healthcare")
    footer.text_frame.text = f"{brand}  |  AI Presentation Generator  |  {number}"
    footer.text_frame.paragraphs[0].font.size = Pt(8)
    footer.text_frame.paragraphs[0].font.color.rgb = _to_rgb(theme["muted"])
...
def add_bullets(slide, items: list[str], x=MARGIN_X, y=CONTENT_Y, width=Inches(11.8), height=Inches(4.9), theme: dict | None = None) -> None:
    ...
    for index, item in enumerate(items):
        ...
        paragraph.font.color.rgb = _to_rgb(theme["ink"])
